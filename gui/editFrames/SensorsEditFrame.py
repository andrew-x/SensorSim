...
class SensorsEditFrame(Frame):
    ...
    def __init__(self, master=None):
        super(SensorsEditFrame, self).__init__(master)
    ...
    def set_widgets(self):
        ...
        self.submit_button.grid(row=len(self.sensors)+1, column=3, pady=10)

    def submit(self):
        tmp = []
        for r in self.rows:
            e = Sensor(r[0].get(), r[1].get(), r[2].get(), r[3].get(), r[4].get(), r[5].get(), r[6].get())
            tmp.append(e)

        Controller.set_sensors(tmp)
